...
import { ASTRO, ASTRO_CONTACT, astroPricing } from '@/lib/astrophotography-data'
...
const schema = z.object({
  ...
  boarding: z.string().min(1, 'Select a package'),
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const res = await fetch(FORMSUBMIT_ENDPOINT, {
        ...
        body: JSON.stringify({
          _subject: `New Astrophotography Expedition booking — ${data.fullName}`,
          _template: 'table',
          _captcha: 'false',
          _replyto: data.email,
          _honey: data.company || '',
          Expedition: ASTRO.name,
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <ScrollReveal className="text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-[var(--av-gold)]/30 bg-[var(--av-gold)]/10 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.25em] text-[var(--av-gold)]">
            {ASTRO.seatsLabel} · Registrations Open
          </span>
          <h2 className="font-display mx-auto mt-5 max-w-2xl text-balance text-4xl font-semibold leading-tight tracking-tight text-white sm:text-5xl">
            Reserve Your Seat on the Expedition
          </h2>
          ...
          <div className="mt-7 flex flex-wrap items-center justify-center gap-2.5">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/[0.06] px-4 py-2 text-sm font-medium text-white/85">
              <Clock size={14} className="text-[var(--av-gold)]" /> {ASTRO.durationLabel}
            </span>
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/[0.06] px-4 py-2 text-sm font-medium text-white/85">
              <MapPin size={14} className="text-[var(--av-gold)]" /> {ASTRO.locationsLabel}
            </span>
          </div>
          ...
          <div className="mx-auto mt-6 grid max-w-xl gap-3 sm:grid-cols-2">
            {astroPricing.map((p) => (
              <div key={p.id} className="rounded-2xl border border-white/12 bg-white/[0.04] p-5 text-left">
            ...
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <div>
                    <label className={labelCls} htmlFor="boarding">Package</label>
                    <select id="boarding" className={cn(inputCls, 'appearance-none')} defaultValue="" {...register('boarding')}>
                      <option value="" disabled className="bg-[var(--av-deep)]">Select a package</option>
                      {astroPricing.map((p) => (
                  ...
                  <div>
                    <label className={labelCls} htmlFor="experience">
                      Experience level <span className="font-normal normal-case text-white/35">(optional)</span>
                    </label>
                    <select id="experience" className={cn(inputCls, 'appearance-none')} defaultValue="" {...register('experience')}>
                      <option value="" className="bg-[var(--av-deep)]">Prefer not to say</option>
                      <option value="Beginner" className="bg-[var(--av-deep)]">Beginner</option>
                      <option value="Intermediate" className="bg-[var(--av-deep)]">Intermediate</option>
                      <option value="Advanced" className="bg-[var(--av-deep)]">Advanced</option>
                    </select>
                  </div>
                </div>

                <label className="flex items-start gap-3 text-sm text-white/70">
                  <input type="checkbox" className="mt-0.5 h-4 w-4 shrink-0 rounded border-white/30 bg-white/10 accent-[var(--av-gold)]" {...register('rental')} />
                  <span>I’m interested in renting equipment (₹2,000–₹3,000 per day, limited availability).</span>
                </label>
                ...
                <div>
                  ...
                  <textarea id="notes" rows={3} className={inputCls} placeholder="Your gear, goals, dietary needs, questions…" {...register('notes')} />
                </div>
                ...
                <label className="flex items-start gap-3 text-xs leading-relaxed text-white/65">
                  ...
                  <span>
                    I agree to be contacted about the Astroventure Astrophotography Expedition and
                    accept that seats and rental gear are confirmed on a first-come basis.
                  </span>
                </label>
